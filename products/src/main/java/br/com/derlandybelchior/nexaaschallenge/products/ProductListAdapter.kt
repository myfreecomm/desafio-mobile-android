...
class ProductListAdapter(private val totalPresentation: TotalProductsPresentation, private val productItemClickListener: ProductItemClickListener) : RecyclerView.Adapter<RecyclerView.ViewHolder>(){

    var items: List<ProductPresentation> = listOf()
    ...
    class ProductListViewHolder(parent: ViewGroup) : RecyclerView.ViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.list_item, parent, false)) {
        fun bind(productPresentation: ProductPresentation, productItemClickListener: ProductItemClickListener) = with(itemView) {
            ...
            loadImage(productPresentation.imageUrl)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (getItemViewType(position)) {
            VIEW_TYPE_HEADER -> {
                (holder as HeaderViewHolder).bind(items.size)
            }
            VIEW_TYPE_FOOTER -> {
